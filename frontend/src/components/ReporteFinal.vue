...
const descargarSabanaOficial = async () => {
  ...
  try {
    const url = `http://localhost:3000/api/excel/descargar-reporte?inicio=${fechaInicioStr}&fin=${fechaFinStr}`;
...
const generarReporteSanciones = async () => {
  ...
  try {
    ...
    /* const url = `http://localhost:3000/api/reportes/sanciones?mes=${mesSeleccionadoSanciones.value}&anio=${anioActual}`;
